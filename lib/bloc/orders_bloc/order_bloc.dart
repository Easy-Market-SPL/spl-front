...
class OrdersBloc extends Bloc<OrdersEvent, OrdersState> {
  ...
  Future<void> _onUpdateOrderAddress(
    ...
    if (updated == null) {
      emit(const OrdersError(
          "It's not possible to update the address of the order"));
      return;
    }
  ...
  Future<void> _onPrepareOrder(
    ...
    if (updated == null) {
      emit(const OrdersError(
          "It's not possible to change the order status to preparing"));
      return;
    }
    add(LoadSingleOrderEvent(event.orderId));
  }
  ...
  Future<void> _onOnTheWayDomiciliary(
    ...
    if (updated == null) {
      emit(const OrdersError(
          "It's not possible to change the order status to on the way."));
      return;
    }
    add(LoadSingleOrderEvent(event.orderId));
  }
  ...
  Future<void> _onOnTheWayTransport(
    ...
    if (updated == null) {
      emit(const OrdersError(
          "It's not possible to change the order status to on the way with a transport company"));
      return;
    }
    add(LoadSingleOrderEvent(event.orderId));
  }
  ...
  Future<void> _onDeliveredOrder(
    ...
    if (updated == null) {
      emit(const OrdersError(
          "It's not possible to change the order status to delivered"));
      return;
    }
    add(LoadSingleOrderEvent(event.orderId));
  }
  ...
  Future<void> _onDeleteOrder(
    ...
    if (!success) {
      emit(const OrdersError("It's not possible to delete the order"));
      return;
    }
  ...
  Future<void> _onUpdateDebt(
    ...
    if (updated == null) {
      emit(const OrdersError("It's not possible to update the order's debet"));
      return;
    }
    add(LoadSingleOrderEvent(event.orderId));
  }
  ...
  // If there's no currentCartOrder, one will be created with the given userId/address.
  //
  Future<void> _onAddProduct(
    ...
    try {
      ...
      if (cartOrder == null || cartOrder.id == null) {
        ...
        if (createError != null || newOrder == null) {
          emit(OrdersError(
              createError ?? "It's not possible to create the order"));
      ...
      if (orderId == null) {
        ...
        emit(const OrdersError("The order ID is null"));
      ...
      if (error != null || updatedOrderData == null) {
        emit(OrdersError(error ?? "It's not possible to update the order"));
        emit(previousState.copyWith(isLoading: false));
        return;
      }
  ...
  Future<void> _onRemoveProduct(
    ...
    if (state is! OrdersLoaded) {
      emit(const OrdersError(
          "Invalidad status for remove the product from the order"));
      return;
    }
    ...
    try {
      ...
      if (err != null || !success) {
        emit(OrdersError(
            err ?? "It's not possible to delete the product from the order"));
        emit(previousState.copyWith(isLoading: false)); // Revert loading
        return;
      }
  ...
  Future<void> _onClearCart(
    ...
    try {
      if (cartOrder != null && cartOrder.id != null) {
        ...
        if (err != null) {
          emit(OrdersError("Error at empty the car on the server: $err"));
  ...
  void _onFilterDeliveryOrders(
    ...
    final filtered = current.allOrders.where((o) {
      if (event.preparacion) {
        ...
        // Orders of the domiciliary with current session
