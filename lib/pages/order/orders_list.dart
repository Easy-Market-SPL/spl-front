...
class OrdersPage extends StatelessWidget {
          ...
          return Column(
            ...
            children: [
              Wrap(
                ...
                runSpacing: 4.0,
                children: [
                  filterChip(context, OrderStrings.statusConfirmed, selected: state.selectedFilters.contains(OrderStrings.statusConfirmed)),
                  filterChip(context, OrderStrings.statusPreparing, selected: state.selectedFilters.contains(OrderStrings.statusPreparing)),
                  filterChip(context, OrderStrings.statusOnTheWay, selected: state.selectedFilters.contains(OrderStrings.statusOnTheWay)),
                  filterChip(context, OrderStrings.statusDelivered, selected: state.selectedFilters.contains(OrderStrings.statusDelivered)),
                ],
              ),
              if (state.additionalFilters.isNotEmpty)
                Wrap(
                  ...
                  runSpacing: 4.0,
                  children: [
                    ...state.additionalFilters.map((filter) => filterChip(context, filter, selected: true, isAditionalFilter: true)),
  ...
  Widget filterChip(BuildContext context, String label, {bool selected = false, bool isAditionalFilter = false}) {
    return ChoiceChip(
      label: Text(label),
      selected: selected,
  ...
  Widget searchBar(BuildContext context) {
    ...
    return SearchBarInput(
      ...
      onFilterPressed: () async {
        ...
        final List<String> currentAdditionalFilters = context.read<OrderListBloc>().state is OrderListLoaded
            ? (context.read<OrderListBloc>().state as OrderListLoaded).additionalFilters
            : [];
        final DateTimeRange? currentDateRange = context.read<OrderListBloc>().state is OrderListLoaded
            ? (context.read<OrderListBloc>().state as OrderListLoaded).selectedDateRange
            : null;
          ...
          builder: (BuildContext context) {
            return FiltersPopup(
              onApplyFilters: (filters, dateRange) {
                context.read<OrderListBloc>().add(ApplyAdditionalFiltersEvent(filters));
