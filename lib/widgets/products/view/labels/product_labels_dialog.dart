                    ...
                    Expanded(
                      child: BlocBuilder<LabelBloc, LabelState>(
                        builder: (context, state) {
                          if (state is LabelLoading) {
                            ...
                            final filteredLabels = searchQuery.isEmpty 
                              ...
                              : allLabels.where((label) => 
                                  label.name.toLowerCase().contains(searchQuery) || 
                                  label.description.toLowerCase().contains(searchQuery)
                                ).toList();
                          ...
                          return const Center(child: Text("Error cargando etiquetas"));
                        },
                      ),
                    ),
                      ...
                      children: [
                        ...
                        Expanded(
                          child: TextButton.icon(
                            onPressed: () async {
                              Navigator.pop(ctx);
                              final newLabel = await showNewLabelDialog(context);
                              if (newLabel != null) {
                                labelBloc.add(LoadLabels());
                              }
                            },
